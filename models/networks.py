...
class GradientICONSparse(network_wrappers.RegistrationModule):
    def __init__(self, network, similarity, lmbda, input_type="image"):
    ...
    def as_function(self, image, spline_order=1):
        ...
        return lambda coordinates: compute_warped_image_multiNC(
            image, coordinates, self.spacing, spline_order
        )
        
    def compute_segmentation_loss(self, phi_AB, phi_BA, segmentation_A, segmentation_B):
        self.phi_AB_vectorfield = phi_AB(self.identity_map)
        self.phi_BA_vectorfield = phi_BA(self.identity_map)
        
        segmentation_A = monai.networks.one_hot(segmentation_A, 4)
        segmentation_B = monai.networks.one_hot(segmentation_B, 4)

        self.warped_image_A = self.as_function(segmentation_A, spline_order=1)(self.phi_AB_vectorfield)
        self.warped_image_B = self.as_function(segmentation_B, spline_order=1)(self.phi_BA_vectorfield)
        
        segmentation_loss = self.similarity(
            self.warped_image_A, segmentation_B
        ) + self.similarity(self.warped_image_B, segmentation_A)
    ...
    def forward(self, image_A, image_B):
        ...
        if self.input_type == "image":
            ...
            similarity_loss = self.compute_segmentation_loss(self.phi_AB, self.phi_BA, image_A, image_B)
...
def make_registration_network(input_shape, include_last_step=False, lmbda=1.5, loss_fn=icon.LNCC(sigma=3), segmentation=False):
    ...
    net = GradientICONSparse(inner_net, loss_fn, lmbda=lmbda, input_type="segmentation" if segmentation else "image")
    net.assign_identity_map(input_shape)
    return net


def train_two_stage_registration(input_shape, batch_function, GPUS, ITERATIONS_PER_STEP, BATCH_SIZE):

    net = make_registration_network(input_shape, include_last_step=False)
    ...
    net_2 = make_registration_network(input_shape, include_last_step=True)
...
def train_single_stage_segmentation_registration(input_shape, batch_function, GPUS, ITERATIONS_PER_STEP, BATCH_SIZE):

    net = make_registration_network(input_shape, include_last_step=False, segmentation=True, loss_fn=monai.losses.DiceLoss(to_onehot_y=False, softmax=False, include_background=True))
...
def make_segmentation_network(in_channels=1, out_channels=4):
